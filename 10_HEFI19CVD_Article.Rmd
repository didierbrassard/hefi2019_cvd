...
# 0) import all survival curves
  survivalcurve_all <- 
    ...
    dplyr::filter(time_to_primary %% 6 == 0 | time_to_primary==max(time_to_primary))
...
survivalcurve_all %>% 
  ...
  dplyr::filter(time_to_primary==max(survivalcurve_all$time_to_primary)) %>% 
...
pOutcome_n_contrast_tab_bysex <-
  ...
  gt::tab_footnote(
    footnote = "P-values for interaction (independent  t-test) reflect the compatibility of the relative risk in males compared with the relative risk in females for a given percentile, with the (null) hypothesis that both relative risks are equivalent under this model.",
...
7.	Brassard D, Elvidge Munene LA, St-Pierre S, Guenther PM, Kirkpatrick SI, Slater J, et al. Development of the Healthy Eating Food Index (HEFI)-2019 measuring adherence to Canada's Food Guide 2019 recommendations on healthy food choices. Appl Physiol Nutr Metab. 2022;47:595-610.
